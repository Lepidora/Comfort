...
def  main():
  ...
  installer = "\installer\Comfort Installer.exe"
   
  if demo:
    installer = "\installer\Comfort Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Comfort manual.pdf" 
  ]
  ...
  ZIPNAME = "Comfort-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Comfort-v" + FULLVERSIONSTR + "-win-demo.zip"
