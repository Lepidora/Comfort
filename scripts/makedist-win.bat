...
if %1 == 1 (echo Making Comfort Windows DEMO VERSION distribution ...) else (echo Making Comfort Windows FULL VERSION distribution ...)
...
REM - msbuild Comfort-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Comfort.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Comfort.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Comfort.aaxplugin\Contents\Win32\Comfort.aaxplugin --out .\build-win\aax\bin\Comfort.aaxplugin\Contents\Win32\Comfort.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Comfort.aaxplugin\Contents\x64\Comfort.aaxplugin --out .\build-win\aax\bin\Comfort.aaxplugin\Contents\x64\Comfort.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Comfort.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Comfort.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Comfort Installer" ".\installer\Comfort Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Comfort Installer.exe" ".\installer\Comfort Demo Installer.exe"
REM -del ".\installer\Comfort Installer.exe"
REM -)
